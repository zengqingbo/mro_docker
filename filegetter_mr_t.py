...
from tasks import handleBigZipfile, handleSingleZipfile
import psycopg2
...
subfoldernor = {
	...
	'ERICSSON' : {'dateDirformat':'%Y%m%d','layer':1,'ext':'.zip'},
...
def getSpfilelist(hour):
	conn = psycopg2.connect(PG_CONN_TEXT)
	cur = conn.cursor()
	cur.execute("select filename from filelist_s where filetime between '{}' and '{}';".format(hour,hour+timedelta(hours=1)))
	rs = cur.fetchall()
...
def mroFileCheck(localdir,filename,type=True):
	dstfilename = os.path.join(localdir,filename)
	if type:
		conn = psycopg2.connect(PG_CONN_TEXT)
		cur = conn.cursor()
		cur.execute("select filename from filelist{} where filename = '{}' ;".format('' if type else '_s',dstfilename))
		rs = cur.fetchall()
		filelist = [r[0] for r in rs]
		cur.close()
		conn.close()
		...
		global spfilelist
		return dstfilename in spfilelist 
...
def mroFileRecord_s(filename,nid):
	conn = psycopg2.connect(PG_CONN_TEXT)
	cur = conn.cursor()
	try:
		m = rg.search(filename)
	...
	d = m.group(1)
	fileDateHour = datetime(int(d[:4]),int(d[4:6]),int(d[6:8]),int(d[8:10]))
	cur.execute("insert into filelist_s(filename,nid,filetime,tasksend) values('{}',{},'{}','{}');".format(filename,nid,fileDateHour,datetime.now()))
	conn.commit()
	cur.close()
	conn.close()
...
def ftpDL(ftp,localdir,filename,nid):
	...
	f = filename.split('_')
	if len(f) > 3 :
		if f[2] in ['ERICSSON','NSN'] :
			handleSingleZipfile.delay(dstname,nid)
			mroFileRecord_s(dstname,nid)
			global spfilelist
			spfilelist.append(dstname)
		else:
...
def niHourGetter(omcinfo,hour):
	if omcinfo['vendor'] in ['ERICSSON','NSN'] :
		global spfilelist 
		spfilelist = getSpfilelist(hour)
		if debuglevel >= 1 : print("spfilelist len: {}, head:{}".format(len(spfilelist),spfilelist[:2]))
	print('nbi id%s ip(%s) is scan' % (omcinfo['id'],omcinfo['ip']))
	ftp = ftpext.FTPExt(omcinfo['ip'],omcinfo['port'],omcinfo['user'],omcinfo['password'])
	ftp.cwd(omcinfo['path'])
	if debuglevel >= 1 : print(os.path.join(omcinfo['path']))
	for k,v in ftp.ls().items():
		...
		k.find(hour.strftime(subfoldernor[omcinfo['vendor']]['dateDirformat'])) != -1 :
			...
			for k1,v1 in ftp.ls().items():
				if subfoldernor[omcinfo['vendor']]['layer'] == 1 :
					if debuglevel >= 2 : print(k1)
					if v1['file'] and isValidMroFile(k1,omcinfo,hour) and not mroFileCheck(localdir,k1,omcinfo['vendor'] not in ['ERICSSON']):
						ftpDL(ftp,localdir,k1,omcinfo['id'])
				elif v1['dir'] and subfoldernor[omcinfo['vendor']]['layer'] == 2 :
					if debuglevel >= 2 : print(omcinfo['path']+'/'+k+'/'+k1)
					ftp.cwd(omcinfo['path']+'/'+k+'/'+k1)
					for k2,v2 in ftp.ls().items():
						if isValidMroFile(k2,omcinfo,hour) and not mroFileCheck(localdir,k2,omcinfo['vendor'] not in ['ERICSSON']):
							ftpDL(ftp,localdir,k2,omcinfo['id'])
	ftp.close()
...
def mrgetter_main():
	if getDiskFreeGB(FTP_DL_DIR) > 500 :
		mroHourFilegetter(hours=getDay(2018,3,27))
		mroHourFilegetter(hours=getDay(2018,3,28))
		mroHourFilegetter(hours=getDay(2018,3,29))
