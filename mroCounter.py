	...
	def ctNcInfo(self,v):
		...
		for ncinfo in v.NcInfo_list:
			...
			if ncrsrp :
				...
				if scrsrp-ncrsrp <= 10 :
					self.ncCounters[key].sumField('nc_rsrp_diff10_counter')
		...
		if pdate + timedelta(days=93) > datetime.now() :
			with open(filename,'w') as f:
				for k ,v in self.scCounters.items():
					line = "%s,%s,%s,%s," % (pdatestr,self.mro.startTime, self.mro.EnodebId, k)
